...
const ItemManagementPage = () => {
  ...
  useEffect(() => {
    if (activeBranch?.id) {
      ...
      setDeptBranchId(activeBranch.id); // Updates DepartmentContext
    }
  }, [activeBranch,  setDeptBranchId]);

  useEffect(() => {
    let unsubscribe;
    const fetchCategoriesData = async () => {
      if (activeBranch?.id && selectedDepartment?.id) {
        setLoadingCategories(true);
        try {
          unsubscribe = fetchCategories(activeBranch?.id, selectedDepartment?.id);
        } finally {
  ...
  }, [activeBranch?.id, selectedDepartment?.id]);
