...
export const PosManagement = () => {
  ...
  const addToCart = (item) => {
    ...
    const currentQuantityOrWeight = existingItem
      ? item.stockManagement.type === "quantity"
    ...
    if (
      currentQuantityOrWeight + incrementAmount <=
      (item.stockManagement.type === "quantity" ? item.stockManagement.quantity : item.stockManagement.totalWeight)
    ) {
      if (existingItem) {
        setCart(
          cart.map((cartItem) =>
  ...
  const updateWeight = (itemId, newWeight) => {
    ...
    const validWeight = Math.max(0, Math.min(newWeight, maxWeight))
  ...
  const calculateTotal = () => {
    return cart.reduce((total, item) => {
      const itemTotal =
        item.stockManagement.type === "weight" ? item.sellingPrice * item.weight : item.sellingPrice * item.quantity
  ...
  const processTransaction = async () => {
    if (!activeBranch?.id || !paymentMethod) {
      toast({
        title: "Error",
    ...
    try {
      ...
      const transactionId = await addTransaction(activeBranch?.id, transactionData)
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <CardDescription>
                {activeBranch
                  ? `Manage sales for ${activeBranch.branchName}`
                          ...
                          {item.stockManagement.type === "weight" ? (
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <Button
                                ...
                                variant="outline"
                                onClick={() => updateQuantity(item.id, item.quantity + 1)}
                                disabled={item.quantity >= item.stockManagement.quantity}
                                className="h-7 w-7"
                              >
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="font-medium">{activeBranch?.branchName}</p>
                    </div>
                    <div className="space-y-1">
