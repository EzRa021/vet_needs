...
const InventoryDashboard = () => {
  ...
  // Set branchId when activeBranch changes
  useEffect(() => {
    if (activeBranch?.id) {
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await deleteItem(itemId, activeBranch?.id);
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <CardDescription>
                {activeBranch
                  ? `Manage inventory items for ${activeBranch.branchName}`
                            ...
                            >
                              {isEditing ? (
                                <Input
                                  ...
                                  onChange={(e) => updateEditedItemField("costPrice", Number(e.target.value))}
                                />
                              ) : (
