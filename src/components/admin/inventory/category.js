...
export default function CategoryForm() {
  ...
  useEffect(() => {
    if (activeBranch?.id && branchId !== activeBranch.id) {
      ...
      setDeptBranchId(activeBranch.id)
    }
  }, [activeBranch, branchId, setBranchId, setDeptBranchId])

  // Set initial department filter only once
  useEffect(() => {
    if (departments.length > 0 && !departmentFilter && activeBranch?.id) {
      const initialDept = departments[0]
      setDepartmentFilter(initialDept)
      setDepartmentId(initialDept.id)
      fetchCategories(activeBranch.id, initialDept.id)
    }
  }, [departments, departmentFilter, activeBranch, fetchCategories, setDepartmentId])
  ...
  const handleDepartmentFilterChange = (value) => {
    ...
    if (newDepartment && newDepartment.id !== departmentFilter?.id) {
      ...
      if (activeBranch?.id) {
        fetchCategories(activeBranch.id, newDepartment.id)
      }
    }
  }
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        fetchCategories(activeBranch.id, selectedDepartment.id)
      }, 300)
    } catch (err) {
  ...
  const handleEditCategory = async () => {
    ...
    try {
      ...
      fetchCategories(activeBranch.id, departmentFilter.id)
    } catch (err) {
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      await deleteCategory(deleteConfirmDialog.id, activeBranch.id, deleteConfirmDialog.departmentId)
      addLog({
  ...
  const handleSyncData = async () => {
    if (!activeBranch || !departmentFilter) {
      toast({
        title: "Error",
      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <CardDescription>
              {activeBranch
                ? `Manage categories for ${activeBranch.branchName}`
          ...
          <div className="flex gap-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    disabled={loadingCategories || !isOnline || !activeBranch || !departmentFilter}
                    variant="outline"
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button
                  disabled={!activeBranch || loadingDepartments || departments.length === 0}
                  className="flex items-center gap-2"
                >
