...
const ItemSenderForm = () => {
  ...
  const selectedBranchName = branches.find(b => b.id === selectedBranch)?.branchName || '';
   ...
   const handleBranchChange = (branchId) => {
    ...
    setDeptBranchId(branchId); // Sync with DepartmentContext
    fetchDepartments(branchId); // Fetch departments
    console.log("Branch changed, fetching departments for:", branchId);
  };

  // Handle department change and fetch categories
  const handleDepartmentChange = (departmentId) => {
    setSelectedDepartment(departmentId);
    setSelectedCategory(""); // Reset category
    if (selectedBranch && departmentId) {
      console.log(
        "Department changed, fetching categories for:",
        selectedBranch,
        departmentId
      );
      fetchCategories(selectedBranch, departmentId); // Fetch categories
    }
  };
    ...
    try {
      ...
      for (const item of parsedItems) {
        ...
        const formattedItem = {
          ...
          branchName: activeBranch ? activeBranch?.branchName : selectedBranchName,
