  ...
  useEffect(() => {
    const loadStoredSettings = () => {
      ...
      if (customFonts && customFonts.length > 0) {
        ...
        styleSheet.id = 'receipt-custom-fonts';

        // Remove any existing stylesheet to prevent duplicates
        const existingStyleSheet = document.getElementById('receipt-custom-fonts');
    ...
    // Optional: Set up a listener for storage changes if you want real-time updates
    // This is useful if the settings might be changed while the receipt is open
    ...
    return () => {
      ...
      const customFontStyle = document.getElementById('receipt-custom-fonts');
      if (customFontStyle) {
