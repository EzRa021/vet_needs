...
export function ReturnsProvider({ children }) {
  ...
  const fetchReturns = useCallback(
    async (transactionId = null) => {
      ...
      try {
        ...
        let returnsData = result.rows
          ...
          .filter((ret) => ret.branchId === activeBranch.id)
    ...
    [localDB, activeBranch?.id, handleError, checkUnsyncedCount],
  )
  ...
  }, [isOnline, activeBranch?.id, localDB, remoteDB, toast, fetchReturns, checkUnsyncedCount])

  const processReturn = async (branchId, transactionId, returnItems, reason) => {
    if (!localDB || !branchId || branchId !== activeBranch?.id) {
  ...
  const updateReturn = async (returnData) => {
    if (!localDB || !activeBranch?.id || returnData.branchId !== activeBranch.id) {
      throw new Error("Invalid or no active branch selected")
    }
  ...
  }, [activeBranch?.id, localDB, fetchReturns, checkUnsyncedCount])
