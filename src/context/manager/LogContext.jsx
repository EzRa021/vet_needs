...
export const LogProvider = ({ children }) => {
  const { userDetails, branches } = useBranches();
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const fetchedLogs = result.rows
        ...
        .filter((log) => log.branchId === userDetails?.branchId)
  ...
  // Add log
  const addLog = async ({ action, message, metadata = {}, logId, branchId = userDetails?.branchId }) => {
    if (!localDB || !branchId) {
  ...
  }, [isOnline, userDetails?.branchId, localDB, remoteDB, toast]);
  ...
  // Fetch logs when userDetails changes
  useEffect(() => {
