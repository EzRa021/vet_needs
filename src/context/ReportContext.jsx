...
export const ReportProvider = ({ children }) => {
  ...
  }, [isOnline, activeBranch?.id, localDB, remoteDB]);
    ...
    async (date) => {
      ...
      try {
        ...
        let fetchedReports = result.rows
          ...
          .filter((rep) => rep.branchId === activeBranch.id);
  ...
  const addReport = useCallback(
    ...
    [activeBranch, localDB, currentPage, toast]
  );
  ...
  const deleteReport = useCallback(
    ...
    [activeBranch, localDB, reports.length, currentPage, toast]
  );
  ...
  // Initial fetch when activeBranch changes
  useEffect(() => {
    if (localDB && activeBranch?.id) {
      const today = new Date();
      fetchReports(today);
    }
  }, [activeBranch, fetchReports, localDB]);
